...
void cTestCPU::Setup(cInstLib * in_inst_lib,  cEnvironment * in_env,
		     const cPopulationInterface & in_interface)
{
  ...
  input_array[0] += 0x93149f;
  input_array[1] += 0x88e53e;
  input_array[2] += 0xe241eb;
...
void cTestCPU::SetInstLib(cInstLib * in_inst_lib)
{
  inst_lib = in_inst_lib;
}
...
bool cTestCPU::TestIntegrity(const cGenome & test_genome)
{
  ...
  static UCHAR copy_id  = inst_lib->GetInstID("copy");
  static UCHAR div_id   = inst_lib->GetInstID("divide");
  static UCHAR alloc_id = inst_lib->GetInstID("allocate");
#endif

#ifdef QUICK_HEAD_TEST_CPU
  // This checks to make sure a 'copy', 'divide', and 'allocate' are all in
  // the creatures, and if not doesn't even bother to test it.
  static UCHAR copy_id  = inst_lib->GetInstID("h-copy");
  static UCHAR div_id   = inst_lib->GetInstID("h-divide");
  static UCHAR alloc_id = inst_lib->GetInstID("h-alloc");
#endif
