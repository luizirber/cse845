...
{
  return (AtEnd()) ? cInstLib::GetInstError() : GetMemory()[position+1];
}
...
{ 
  ...
  const cInstLib & inst_lib = main_hardware->GetInstLib();
  ...
  while (pos < search_mem.GetSize()) {
	...
	for (matches = 0; matches < label_size; matches++) {
	  if (search_label[matches] !=
	      inst_lib.GetNopMod( search_mem[offset + matches] )) {
	    break;
	  }
	}
...
{ 
  ...
  while (pos >= 0) {
    ...
    if (inst_lib.IsNop( search_mem[pos] )) {
      ...
      int start_pos = pos;
      int end_pos = pos + 1;
      while (start_pos > 0 && inst_lib.IsNop(search_mem[start_pos - 1])) {
	start_pos--;
      }
