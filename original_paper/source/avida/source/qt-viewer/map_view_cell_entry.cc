...
void
AgeColorUtil::setupColorScale( bool force_redraw = false )
{
  if (force_redraw){
...
void
ParasiteColorUtil::setupColorScale( bool force_redraw = false )
{
  if (force_redraw){
    m_color_scale_widget->setColorList( m_color_vector, m_name_vector );
    m_color_scale_widget->activate();
  ...
  m_pop_wrap( pop_wrap )
{
  ...
  vector<int>::iterator it = m_genotype_id_vector.begin();
  for ( ; it != m_genotype_id_vector.end(); it++)
    *it = -1;
  m_name_vector.resize( m_max_color );
  vector<QString>::iterator it2 = m_name_vector.begin();
...
GenotypeColorUtil::setupColorScale( bool )
{
  ...
  m_pop_wrap->lock();
  cGenotype *temp_gen =
    m_pop_wrap->getPopulation()->GetGenebank().GetBestGenotype();
  m_pop_wrap->unlock();
...
void
LineageColorUtil::setupColorScale( bool force_redraw = false )
{
  if (force_redraw){
    m_color_scale_widget->setColorList( m_color_vector, m_name_vector );
    m_color_scale_widget->activate();
...
MapViewCellColorUtil::MapViewCellColorUtil(
  ...
  : m_color_scale_widget( cs_widget ),
    ...
    m_genotype_color_util( pop_wrap, cs_widget ),
...
MapViewCellColorUtil::calcColorScale( bool force_redraw )
{
  ...
  case MERIT:
    ...
    setupColorScale(
      0,
      m_pop_wrap->getPopulation()->GetStats().GetMaxMerit(),
      force_redraw
    );
    m_pop_wrap->unlock();
    break;
  case FITNESS:
    ...
    setupColorScale(
      0,
      m_pop_wrap->getPopulation()->GetStats().GetMaxFitness(),
      force_redraw
    );
    m_pop_wrap->unlock();
    break;
