  ...
  m_genesis(0)
{
  ...
  m_log_geneology = new cSetupIntChoice(
    "LOG_GENEOLOGY",
    "",
    GeneologyLoggingOff,
    true,
  ...
  */
  ...
  insertSetupDescription(m_log_geneology);
  ...
  insertSetupDescriptionMapping(m_log_geneology);
...
GenesisWrapper::setGenesis(cGenesis *genesis){
  Message("<GenesisWrapper::setGenesis>");
  cGenesis *old_genesis = m_genesis;
  m_genesis = genesis;
  if(old_genesis) {
    Message(" --- delete old_genesis");
    delete old_genesis;
  }
}
...
GenesisWrapper::saveGenesis(const QString &genesis_filename){
  Message("<GenesisWrapper::saveGenesis>");
