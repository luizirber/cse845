...
N_Instruction_ScrollView::contentsMousePressEvent(QMouseEvent *event){
  //#if LOCAL_DEBUG
  cout
  << "<N_Instruction_ScrollView::contentsMousePressEvent>\n"
  << " --- x() " << event->x() << ", y() " << event->y() << ".\n"
  << " --- corresponds to line "
  ...
  //#if LOCAL_DEBUG
  cout
  << "<N_Instruction_ScrollView::contentsMousePressEvent> done.\n";
  //#endif
}
...
N_Instruction_ScrollView::contentsMouseReleaseEvent(QMouseEvent *event){
  ...
  */
  ...
  //#if LOCAL_DEBUG
  cout
  << "<N_Instruction_ScrollView::contentsMouseReleaseEvent> done.\n";
  //#endif
}
...
N_Instruction_ScrollView::contentsMouseMoveEvent(QMouseEvent *event){
  #if LOCAL_DEBUG
  cout
  << "<N_Instruction_ScrollView::contentsMouseMoveEvent>\n"
  << " --- x() " << event->x() << ", y() " << event->y() << ".\n"
  << " --- corresponds to line "
  << (event->y() / s_button_height) << ".\n";
  #endif
  ...
  */
  ...
  #if LOCAL_DEBUG
  cout
  << "<N_Instruction_ScrollView::contentsMouseMoveEvent> done.\n";
  #endif
}
...
N_Instruction_ScrollView::leaveEvent(QEvent *e){
  #if LOCAL_DEBUG
  cout
  << "<N_Instruction_ScrollView::leaveEvent> entered.\n";
  #endif
  ...
  #if LOCAL_DEBUG
  cout
  << "<N_Instruction_ScrollView::leaveEvent> done.\n";
  #endif
}
...
N_Instruction_ScrollView::locateButton(int x){
  ...
  #if LOCAL_DEBUG
  cout
  << "<N_Instruction_ScrollView::locateButton>\n"
  << "x " << x << ", located_button " << located_button << ".\n";
  #endif
...
N_Instruction_ScrollView::drawButtonFrame(
  ...
  #if LOCAL_DEBUG
  cout
  << "<N_Instruction_ScrollView::drawButtonFrame2>\n"
...
N_Instruction_ScrollView::undrawLastButtonFrame(void){
  ...
  if(
    ...
    */
    ...
    default:
      qFatal(
        "<N_Instruction_ScrollView::undrawLastButtonFrame> bad m_highlighting_index."
      );
      break;  // not reached.
    }
    p.setPen(
  ...
  if(line->m_il_is_dirty){
    ...
    if(it == s_instr_pm_set.end()){
      buildPixmapPair(
        ...
        //QString(inst_lib.GetName(m_last_instruction)()),
