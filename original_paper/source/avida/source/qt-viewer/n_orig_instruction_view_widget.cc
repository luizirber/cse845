...
void
N_Instruction_ViewWidget::setPopulationWrapper(
...
N_Instruction_ViewWidget::checkInstrLinesListSize(int size){
  ...
  if(counter < size){
    ...
    cHardwareCPU *hardware =
      GetHardwareCPU(
        GetOrganism(
          GetPopulationCell(m_pop_wrap, m_cell_id)
        )
      );

    if(hardware != NULL){
      memory = hardware->IP().GetMemory();
      inst_lib = hardware->GetInstLib();
    }
...
N_Instruction_ViewWidget::updateInstrSubmenu(QPopupMenu *submenu){
  ...
  if(hardware){
    cInstLib inst_lib = hardware->GetInstLib();
    for(i = 0; i < inst_lib.GetSize(); i++){
      QString instr_i_text(inst_lib.GetName(i)());
    ...
    if(inst_lib.GetSize() < menu_item_count){
