...
inline void cHistogram::Insert(int value, int count)
{
  if (value > max_bin || value < min_bin) {
    cerr << "Trying to insert " << value << " into Histogram of range ["
	 << min_bin << "," << max_bin << "]" << endl;
  }
...
#ifdef DEBUG
  if (value > max_bin || value < min_bin) {
    cerr << "Trying to remove " << value << " into Histogram of range ["
	 << min_bin << "," << max_bin << "]" << endl;
    return;
  }
  if (bins[value] <= 1) {
    cerr << "WARNING: Trying to remove " << value << " from bin with "
	 << bins[value] << " entries" << endl;
    return;
  }
#endif
...
#ifdef DEBUG
  if (value > max_bin || value < min_bin) {
    cerr << "WARNING: Trying to remove " << value
