...
void cAnalyze::CommandPrintPhylogeny(cString cur_string)
{
  cerr << " WARNING: PrintPhylogeny is not currently working!" << endl;

  // Print out a file that gives the full backbone of a phylogenetic tree.
  // This assumes that we have loaded the final population and all of its
  // ancestors...

  // @CAO Lets do this brute-force for now...
  // Loop through all genotypes and plot its coordinates with its parent.
  cAnalyzeGenotype * child_gen = NULL;
  tListIterator<cAnalyzeGenotype> child_it(batch[cur_batch].List());
  while ((child_gen = child_it.Next()) != NULL) {
  }
...
void cAnalyze::BatchRecalculate(cString cur_string)
{
  ...
  if (verbose == true && batch[cur_batch].IsLineage() == false) {
    cerr << "  Warning: batch may not be a linege; "
	 << "parent distances may not be correct" << endl;
  }
