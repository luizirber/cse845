...
private:
  ...
  cInstLib & inst_lib;       // Instruction Library for this genome
...
public:
  cAnalyzeGenotype(cString symbol_string, cInstLib & in_inst_lib);
  cAnalyzeGenotype(const cGenome & _genome, cInstLib & in_inst_lib);
...
private:
  ...
  // Comamnd helpers...
  void CommandDetail_Header(ostream & fp, int format_type,
	    tListIterator< tDataEntryCommand<cAnalyzeGenotype> > & output_it);
  void CommandDetail_Body(ostream & fp, int format_type,
	    tListIterator< tDataEntryCommand<cAnalyzeGenotype> > & output_it);
private:
  // Loading methods...
  ...
  void CommandPrintPhylogeny(cString cur_string);
