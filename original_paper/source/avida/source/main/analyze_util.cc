...
void cAnalyzeUtil::TestGenome(const cGenome & genome, cInstLib & inst_lib,
...
{
  cCPUTestInfo test_info;
  const cInstruction inst_none = inst_lib.GetInst("instruction_none");
  ...
  for (int i = 0; i < num_trials; i++) {
    ...
    while (ins_pos < 0) {
      ...
      if( inst_lib.IsNop(genome[ins_pos]) )  ins_pos = -1;
    }
...
void cAnalyzeUtil::AnalyzeLandscape(const cGenome & genome, cInstLib &inst_lib,
	       int sample_size, int min_found, int max_sample_size, int update)
{
  ...
  for (int dist = 1; dist <= 10; dist++) {
    ...
    if (dist == 1) {
      landscape.Process(dist);
      num_found = genome.GetSize() * (inst_lib.GetSize() - 1);
    } else {
      num_found =
...
void cAnalyzeUtil::PairTestLandscape(const cGenome &genome, cInstLib &inst_lib,
...
{
  const int num_inst = population->GetEnvironment().GetInstLib().GetSize();
	    ...
	    double sample_prob, bool landscape, bool save_genotype)
{
  ...
  for (int i = 0; i < pop->GetSize(); i++) {
    ...
    if (landscape &&  genotype->GetTestFitness() > 0) {
      cLandscape landscape( genome, pop->GetEnvironment().GetInstLib());
      landscape.Process(1);
      landscape.PrintStats(fp);
    }
    else fp << endl;
