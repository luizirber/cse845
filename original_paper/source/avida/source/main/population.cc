  ...
  , sync_events(false)
{
  ...
  cout
  << " --- cConfig::GetWorldX() "
  << cConfig::GetWorldX() << endl;
  world_y = cConfig::GetWorldY();
  cout
  << " --- cConfig::GetWorldY() "
  << cConfig::GetWorldY() << endl;
  const int num_cells = world_x * world_y;
  cout
  << " --- num_cells "
  << num_cells << endl;

  printf("...Creating Organisms...\n");
  ...
  for (int cell_id = 0; cell_id < num_cells; cell_id++) {
    ...
    conn_list.Push( &(cell_array[Mod(cell_id -world_x -1, num_cells)]) );
    conn_list.Push( &(cell_array[Mod(cell_id -world_x   , num_cells)]) );
    conn_list.Push( &(cell_array[Mod(cell_id -world_x +1, num_cells)]) );
    conn_list.Push( &(cell_array[Mod(cell_id          +1, num_cells)]) );
    conn_list.Push( &(cell_array[Mod(cell_id +world_x +1, num_cells)]) );
    conn_list.Push( &(cell_array[Mod(cell_id +world_x   , num_cells)]) );
    conn_list.Push( &(cell_array[Mod(cell_id +world_x -1, num_cells)]) );
    conn_list.Push( &(cell_array[Mod(cell_id          -1, num_cells)]) );
  ...
  for (int i = 0; i < resource_lib.GetSize(); i++) {
    ...
    resource_count.Setup(i, res->GetInitial(), res->GetInflow(), decay);
  }
				    ...
				    cOrganism & parent_organism)
{
  ...
  LineageSetupOrganism( child_organism, parent_organism.GetLineage(),
				   ...
				   cPopulationCell & target_cell)
{
  ...
  // Randomize the inputs in the target cell.
  target_cell.RandomizeInputs();
				...
				tList<cPopulationCell> & found_list)
{
  ...
  found_list.Push(&parent_cell);
  double min_ratio = parent_cell.GetOrganism()->CalcMeritRatio();
